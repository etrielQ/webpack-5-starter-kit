module.exports = class Page {
  ...
  ajax(obj) {
    ...
    a.getHeaders = [["X-Requested-With", "XMLHttpRequest"]];

    a = Object.assign(a, obj);
    a.method = a.method.toUpperCase();

    if (typeof a.data === "string") a.dataString = encodeURIComponent(a.data);
    else
      for (let item in a.data)
        a.dataString += item + "=" + encodeURIComponent(a.data[item]) + "&";

    let xhReq = new XMLHttpRequest();
    if (window.ActiveXObject) xhReq = new ActiveXObject("Microsoft.XMLHTTP");

    if (a.method == "GET") {
      if (typeof a.data !== "undefined" && a.data !== null)
        a.url = a.url + "?" + a.dataString;
      xhReq.open(a.method, a.url, a.async);
      for (let x = 0; x < a.getHeaders.length; x++)
        xhReq.setRequestHeader(a.getHeaders[x][0], a.getHeaders[x][1]);
      xhReq.send(null);
    } else {
      xhReq.open(a.method, a.url, a.async);
      for (let x = 0; x < a.postHeaders.length; x++)
        xhReq.setRequestHeader(a.postHeaders[x][0], a.postHeaders[x][1]);
